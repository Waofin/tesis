...
try:
    ...
    import warnings
    warnings.warn("scipy not available, falling back to numpy.linalg")
...
def pixelCalc(patch):
    ...
    OPTIMIZED VERSION: Uses scipy.linalg.inv() with regularization for better performance and stability
    ...
    # Calculate the covariance matrix (now vectorized)
    S = sampleCovariance(patch, m, T)
    rho = shrinkageFactor(S, T) 
    F = diagSampleCovariance(S)
    C = covariance(rho, S, F)
    
    # OPTIMIZATION: Use scipy.linalg.inv() with regularization for better performance and stability
    # Regularization helps with numerical stability when C is near-singular
    try:
        if SCIPY_AVAILABLE:
            ...
            reg = 1e-8 * np.eye(C.shape[0])
            Cinv = np.linalg.inv(C + reg)
    except (linalg.LinAlgError, np.linalg.LinAlgError):
        # If matrix is singular, use pseudoinverse as fallback
        if SCIPY_AVAILABLE:
            Cinv = linalg.pinv(C)
        else:
...
def sampleCovariance(r, m, T):
    ...
    OPTIMIZED VERSION: Vectorized implementation for better performance
    ...
    """
    # OPTIMIZATION: Vectorized version replaces list comprehension
    # Original: S = (1.0/T)*np.sum([np.cov(np.stack((p, m)), rowvar = False, bias = False) for p in r],axis = 0)
    # New: Direct vectorized computation using broadcasting
    
    # Center the patches by subtracting the mean
    r_centered = r - m[np.newaxis, :]  # Broadcasting: (T, size) - (size,) -> (T, size)
    
    # Compute covariance matrix: (1/T) * sum((p-m)^T * (p-m))
    # Using matrix multiplication: r_centered.T @ r_centered
...
def shrinkageFactor(S, T):
    ...
    OPTIMIZED VERSION: Vectorized computation of diagonal elements
    ...
    T : int
        Number of temporal frames
    """
    # OPTIMIZATION: Replace list comprehension with vectorized operations
    # Original: np.sum(np.array([d**2.0 for d in np.diag(S)]))
    # New: Direct vectorized computation
    diag_S = np.diag(S)
    diag_S_squared = diag_S ** 2.0
    
    top = (np.trace(np.dot(S,S)) + np.trace(S)**2 - 2.0*np.sum(diag_S_squared))
    bot = ((T+1.0)*(np.trace(np.dot(S,S))-np.sum(diag_S_squared)))
    p = top/bot
