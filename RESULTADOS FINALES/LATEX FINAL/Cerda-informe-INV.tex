...
\title{Detección y caracterización de exoplanetas mediante la paralelización del algoritmo paco en observación directa\\
    Curso de Proyecto de Título}
...
\input{capitulos/1introduccion}

\chapter{Bases Teóricas y Antecedentes en los que se basa la Investigación} \label{cap:marco_teorico}
\input{capitulos/2marco}

\chapter{Estado del Arte} \input{capitulos/3estado}
\label{cap:estado_del_arte}

\chapter{Desarrollo del Trabajo y Metodología} \label{cap:metodologia}
\input{capitulos/4metodologia}

\chapter{Analisis de Algoritmos}  \label{cap:algoritmos}
\input{capitulos/5Algoritmos}

\chapter{Resultados}  \label{cap:aplicacion}
\input{capitulos/6resultados}

\chapter{Conclusiones y Trabajo Futuro} \label{cap:conclusion}
\input{capitulos/7conclusiones}
